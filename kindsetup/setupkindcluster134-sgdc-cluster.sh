...
kind create cluster --config ../kindconfig/kindconfig-v134-sgdc-cluster.yaml
sleep 1
kubectl apply -f https://raw.githubusercontent.com/metallb/metallb/v0.15.3/config/manifests/metallb-native.yaml --context kind-sgdc-cluster
pods_checking=false
...
while ! $pods_checking; do
    ...
    if kubectl get pods -n metallb-system | grep -i controller | grep -q 'Running' && kubectl get pods -n metallb-system | grep -i speaker | grep -q 'Running' ; then
        ...
        kubectl create -f ../kindconfig/metallb-v0153-ipaddress-pool-1.yaml --context kind-sgdc-cluster
