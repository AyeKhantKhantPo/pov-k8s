...
kind create cluster --config ../kindconfig/kindconfig-v134-audc-cluster.yaml
sleep 1
kubectl apply -f https://raw.githubusercontent.com/metallb/metallb/v0.15.2/config/manifests/metallb-native.yaml --context kind-audc-cluster
pods_checking=false
...
while ! $pods_checking; do
    ...
    if kubectl get pods -n metallb-system | grep -i controller | grep -q 'Running' && kubectl get pods -n metallb-system | grep -i speaker | grep -q 'Running' ; then
        ...
        kubectl create -f ../kindconfig/metallb-v0152-ipaddress-pool-5.yaml --context kind-audc-cluster
...
sleep 1
kubectl config rename-context kind-audc-cluster audc-cluster

# watch kubectl get pods -A --context audc-cluster
